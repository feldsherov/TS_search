...
class QueryHandler:
    ...
    def parse_query(self, query):
        ...
        while len(query_st) > 1:
            ...
            if ps != "!":
                ...
                opr = operations_dict[ps]
                query_st[-1] = opr(query_st[-1])
        return query_st[-1] if query_st else QTree(word="")
    ...
    def get_records(self, query):
        qtree = self.parse_query(query)
        return self.execute_query(qtree)

    @staticmethod
    def or_(iter1, iter2):
        return itertools.imap(lambda tp: reduce(QueryResult.add_record, tp[1], QueryResult()),
...
def rank_results(query_results, cnt_docs, order_by="bm25"):
...
def main():
    ...
    query_results = handler.get_records(query)

    if options.mode == "b":
        print "\n".join(reader.get_urls_by_ids(map(lambda a: a.doc_id, query_results)))
    else:
        query_results = rank_results(query_results, reader.get_urls_cout(), options.mode)
        print "\n".join(reader.get_urls_by_ids(map(lambda a: a.doc_id, query_results)))
